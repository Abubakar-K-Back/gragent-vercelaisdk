...
export default class Create extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (flags.system) spec.system = flags.system;
    if (flags.steps) spec.steps = flags.steps;
    if (flags.mcp) spec.useMcp = true;
...
`,
  ...
  private modelSnippet(provider: LLMProvider, model?: string): { importLine: string; modelLine: string } {
